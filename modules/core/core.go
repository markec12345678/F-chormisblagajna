...
import (
	...
	common_middlewares "github.com/nutrixpos/pos/common/middlewares"
...
func (c *Core) RegisterHttpHandlers(router *mux.Router, prefix string) {
	...
	if c.Config.Auth.Enabled {
		...
		router.Handle(prefix+"/api/auth/login", common_middlewares.RateLimit(10, time.Minute)(core_middlewares.AllowCors(http.HandlerFunc(authHandler.Login)))).Methods("POST", "OPTIONS")
		router.Handle(prefix+"/api/auth/register", common_middlewares.RateLimit(5, time.Minute)(core_middlewares.AllowCors(http.HandlerFunc(authHandler.Register)))).Methods("POST", "OPTIONS")
