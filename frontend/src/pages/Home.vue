      ...
      <template #center>
        <IconField iconPosition="left">
          ...
          <InputText
            ...
            @keyup.stop="mainSearchTextChanged"
          />
        </IconField>
        <OverlayPanel
          ref="mainsearch_op"
    ...
    <div class="grid m-0 p-0" style="height: calc(94vh - 1.5rem); flex-shrink: 0">
      <div class="col-2 flex flex-column py-3">
        <div
          ...
          :key="index"
          :style="`background-color:${store.getColorMode == 'light' ? 'white' : '#18181B'};cursor:pointer`"
          @click="selectedCategory = category"
        >
          <div
            :style="`width:0.5rem;background-color:${category == selectedCategory ? '#FDDB00' : 'silver'}`"
          ></div>
          <div
            class="py-3 mx-3"
            :style="`color:${category == selectedCategory ? (store.getColorMode == 'light' ? '#2E4762' : '#FDDB00') : store.getColorMode == 'light' ? 'black' : 'white'};font-weight:${category == selectedCategory ? 'bold' : '200'}`"
          >
            {{ category.name }}
          </div>
        </div>
        ...
        <Card class="w-12" :style="`border-color: ${is_order_valid ? '' : 'red'};`">
          <template #content>
            <div
              ...
              style="
                height: calc(94vh - 1.5rem - 36px - var(--p-card-body-padding));
                flex-shrink: 0;
              "
            >
              ...
              <div class="flex flex-column flex-wrap justify-content-between">
                <div>
                    ...
                    <div class="flex flex-column gap-2" style="width: 25vw">
                      ...
                      <div class="flex w-full">
                        <Slider
                          ...
                          style="height: 0.6rem"
                        />
                        <p class="ml-2" style="font-size: 0.8rem">
                  ...
                  <div class="flex justify-content-between flex-wrap align-items-center">
                    ...
                    <p style="font-size: 0.7rem">
                      {{ subtotal.toFixed(2) }}
                      <span style="font-size: 0.7rem">{{ t('egp') }}</span>
                    </p>
                  </div>
                  <div class="flex justify-content-between flex-wrap align-items-center">
                  ...
                  <div class="flex justify-content-between flex-wrap align-items-center">
                    ...
                    <p style="font-size: 1.4rem">
                      {{ total.toFixed(2) }} <span style="font-size: 1rem">{{ t('egp') }}</span>
                    </p>
                  </div>
                </div>
                <div class="flex flex-column align-items-start mb-3">
                  <div class="flex justify-content-center align-items-center mt-2 flex-wrap gap-2">
                    <ToggleButton
                      ...
                      v-model="is_print_receipt_kitchen"
                      :onLabel="$t('kitchen')"
                      :offLabel="$t('kitchen')"
                      onIcon="fa fa-print"
                      offIcon="fa fa-print"
                    ...
                    <ToggleButton
                      size="small"
                      v-model="is_print_receipt_client"
                      :onLabel="
                        store.getShopMode === 'kitchen' ? $t('client') : $t('print_receipt_enabled')
                      "
                      :offLabel="
                        store.getShopMode === 'kitchen'
                          ? $t('client')
                          : $t('print_receipt_disabled')
                      "
                      onIcon="fa fa-print"
                      offIcon="fa fa-print"
                      class="w-36 mx-1"
                ...
                <Button
                  :label="$t('next')"
                  :icon="`pi pi-arrow-${store.orientation == 'rtl' ? 'left' : 'right'}`"
                  iconPos="right"
      ...
      <Dialog
        v-model:visible="edit_item_dialog"
        modal
        :header="$t('edit_order_item')"
        class="xs:w-12 md:w-10 lg:w-8"
      >
        <OrderItemView v-model="orderItems[itemToEditIndex]" />
      </Dialog>
      <Dialog
        v-model:visible="order_details_dialog"
        modal
        :header="$t('order_details')"
        class="w-11 xl:w-8"
      >
        <OrderView
          @updated="order_details_dialog = false"
      ...
      <Dialog
        v-model:visible="order_additional_details_dialog"
        modal
        :header="$t('order_details')"
        class="xs:w-12 md:w-10 lg:w-8"
        :dir="store.orientation == 'rtl' ? 'rtl' : 'ltr'"
      >
        <Stepper linear value="1">
          <StepList>
            <Step
              v-for="(step, index) in order_details_steps"
              :key="index"
              :value="`${index + 1}`"
              >{{ step.label }}</Step
            >
          ...
          <StepPanels>
            <StepPanel v-slot="{ activateCallback }" value="1">
              <div class="grid">
                <div
                  :class="`flex flex-column align-items-center ${store.getShopMode === 'kitchen' ? 'col-12 md:col-4' : 'col-12'}`"
                >
                  <h2 class="mt-0">
                ...
                <template v-if="store.getShopMode === 'kitchen'">
                  <div class="flex flex-column align-items-center col-12 md:col-4">
                    ...
                    <ToggleButton
                      v-model="is_serve_inside"
                      onIcon="fa fa-bowl-food"
                      offIcon="fa fa-bowl-food"
                      :offLabel="$t('dine_in')"
                      :onLabel="$t('dine_in')"
                      class="w-15rem h-5rem lg:h-10rem sm:w-40 border-noround"
                      aria-label="Confirmation"
                ...
                <div class="flex flex-column align-items-start col-12 md:col-4">
                  ...
                  <h2 class="mt-6 mb-0 w-full align-items-start justify-content-start">
                    ...
                    <Button
                      icon="pi pi-pencil"
                      severity="primary"
                      @click="pick_customer_dialog = true"
                    />
                  </h2>
                  ...
                  <div class="flex flex-column">
                    <div
                      v-for="(item, index) in custom_data"
                      :key="index"
                      class="flex align-items-start flex-column gap-1"
                    >
                      <span>{{ $t('key') }}:</span>
                      ...
                      <Button
                        ...
                        :aria-label="$t('remove')"
                        icon="pi pi-times"
                        @click="custom_data.splice(index, 1)"
                      />
                    </div>
              ...
              <div class="flex pt-6 justify-content-end">
                <Button
                  :label="$t('next')"
                  :icon="`pi pi-arrow-${store.orientation == 'rtl' ? 'left' : 'right'}`"
                  :iconPos="`${store.orientation == 'rtl' ? 'left' : 'right'}`"
            ...
            <StepPanel
              ...
              v-if="order_details_steps.length == 3"
              value="2"
            >
              <div class="flex flex-column">
            ...
            <StepPanel
              v-slot="{ activateCallback }"
              :value="order_details_steps.length == 3 ? '3' : '2'"
            >
              <div class="flex flex-column">
                <h2 class="mt-0">
                ...
                <div
                  class="flex align-items-start mt-3 gap-1"
                  v-if="store.getShopMode === 'kitchen'"
                >
                  <span>{{ $t('autostarting') }}:</span>
                  <p class="my-0">
                    <strong> {{ is_auto_start_order }} </strong>
                  </p>
                </div>
                <div class="flex align-items-start mt-3 gap-1">
                  <span>{{ $t('payment') }}:</span>
                  <p class="my-0">
                    <strong> {{ is_pay_later ? $t('pay_later') : $t('now') }} </strong>
                  </p>
                </div>
                <div class="flex align-items-start mt-3 gap-1">
                  <span>{{ $t('payment_source') }}:</span>
                  <p class="my-0">
                    <strong> {{ payment_source?.name }} </strong>
                  </p>
                </div>
                <div class="flex align-items-start mt-3 gap-1">
                  <span>{{ $t('location') }}:</span>
                  <p class="my-0" v-if="is_serve_inside">
                    <strong> {{ $t('dine_in') }} </strong>
                  </p>
                  <p class="my-0" v-if="is_delivery">
                    <strong> {{ $t('delivery') }} </strong>
                  </p>
                  <p class="my-0" v-if="is_take_away">
                    <strong> {{ $t('take_away') }} </strong>
                  </p>
                </div>
                <div class="flex align-items-start mt-3 gap-1">
                  <span>{{ t('comment') }}:</span>
                  <p class="my-0">
                    <strong> {{ order_comment }} </strong>
                  </p>
                </div>
                <div class="flex flex-column align-items-start mt-3 gap-1">
                  <span>{{ $t('other') }}:</span>
            ...
            <li>
              ...
              >
                ...
                <div class="flex flex-column-reverse">
                  <NotificationView
                    @closed="notifications.splice(index, 1)"
              ...
              >
                <Panel :header="t('chats')">
                  <div style="height: 40vh; overflow-y: auto" ref="chat_container">
                    <div
                      ...
                      :key="index"
                      :class="`flex ${chat.user_sub == user.sub ? 'justify-content-end' : ''}`"
                    >
                      <InlineMessage
                        severity="success"
                        v-if="chat.user_sub != user.sub"
                        class="mt-2"
                      >
                        <template #default>
                ...
                <InputGroup class="mt-2">
                  <InputText
                    ...
                    @keyup.enter="SendChatMessage(chat_text)"
                  />
...
const increaseOrderItemQty = (index: number) => {
  const currentQuantity = orderItems.value.reduce((total, current) => {
    return current.product.id == orderItems.value[index].product.id
  ...
  if (
    product &&
    product.enable_inventory_consumption &&
    product.availability < currentQuantity + 1
  ) {
    toast.add({
      severity: 'warn',
  ...
  orderItems.value[index].price =
    orderItems.value[index].quantity * orderItems.value[index].product.price
}
...
watch(is_collecting_money, (new_val) => {
  if (new_val) is_pay_later.value = false
})

watch(is_pay_later, (new_val) => {
  if (new_val) is_collecting_money.value = false
})
...
const getCurrentOrders = () => {
  ...
  axios
    .get(
      `http://${import.meta.env.VITE_APP_BACKEND_HOST}${import.meta.env.VITE_APP_MODULE_CORE_API_PREFIX}/api/orders?filter[is_paid]=false&filter[state]=!stashed`,
      {
        headers: {
          Authorization: `Bearer ${auth.accessToken.value}`,
        },
  ...
  axios
    .get(
      `http://${import.meta.env.VITE_APP_BACKEND_HOST}${import.meta.env.VITE_APP_MODULE_CORE_API_PREFIX}/api/orders?filter[is_paid]=true&filter[state]=in_progress&filter[state]=pending&filter[state]=!stashed`,
      {
        headers: {
          Authorization: `Bearer ${auth.accessToken.value}`,
        },
...
const BackStashedOrderToCheckout = async (stashed_order_index: number) => {
  ...
  discount_percent.value = isNaN((order.discount * 100) / tmp_subtotal)
    ? 0
    : (order.discount * 100) / tmp_subtotal
...
const getStashedOrders = () => {
  axios
    .get(
      `http://${import.meta.env.VITE_APP_BACKEND_HOST}${import.meta.env.VITE_APP_MODULE_CORE_API_PREFIX}/api/orders?filter[state]=stashed`,
      {
        headers: {
          Authorization: `Bearer ${auth.accessToken.value}`,
        },
    ...
    .then(async (response) => {
      ...
      for (let i = 0; i < dataCopy.length; i++) {
        ...
        for (let j = 0; j < dataCopy[i].items.length; j++) {
...
const SendChatMessage = (msg: string) => {
  if (socket.readyState === WebSocket.OPEN) {
    socket.send(
      `{"type":"chat_message","message":"${msg}","sender_name":"${user.value.name}","user_sub":"${user.value.sub}","to":"*","date": "${new Date().toLocaleString()}"}`,
    )
  }
  chat_text.value = ''
}
  ...
  await axios
    ...
    .then(async (response) => {
      payment_sources.value =
        response.data.data.payment_sources == null ? [] : response.data.data.payment_sources
...
const addItem = async (item) => {
  ...
  orderItems.value.forEach((order_item) => {
    if (order_item.product.id == item.id && order_item.comment == '') {
      // if same product with no comment, increase qty instead of adding new line
      order_item.quantity++
...
const submitOrder = () => {
  ...
  const order: SubmitOrderPayload = {
    ...
    customer:
      new_order_delivery_customer.value.length > 0 ? new_order_delivery_customer.value[0] : null,
        ...
        `http://${import.meta.env.VITE_APP_BACKEND_HOST}${import.meta.env.VITE_APP_MODULE_CORE_API_PREFIX}/api/orders`,
        {
          meta: {
            ...
            is_print_kitchen_receipt:
              is_print_receipt_kitchen.value && store.getShopMode === 'kitchen',
          },
          data: order,
        },
        {
...
watch(searchtext, (newSearchText) => {
  ...
  axios
    .get(
      `http://${import.meta.env.VITE_APP_BACKEND_HOST}${import.meta.env.VITE_APP_MODULE_CORE_API_PREFIX}/api/products?filter[search]=${newSearchText}`,
      {
        headers: {
          Authorization: `Bearer ${auth.accessToken.value}`,
        },
...
watch(discount, (new_discount) => {
  if (!isUpdatingDiscountPercent.value) {
    ...
    if (new_discount != 0) discount_percent.value = (new_discount * 100) / subtotal.value
    if (total.value < 0) total.value = 0
  } else {
...
const refreshAvailabilities = () => {
  ...
  if (products.value != null) {
    ...
    axios
      .get(
        `http://${import.meta.env.VITE_APP_BACKEND_HOST}${import.meta.env.VITE_APP_MODULE_CORE_API_PREFIX}/api/products/availability?ids=` +
          product_ids,
        {
          headers: {
            Authorization: `Bearer ${auth.accessToken.value}`,
      ...
      .then((response) => {
        products.value.forEach((product, index) => {
          products.value[index].availability =
            Math.round(
              response.data.data.filter((x) => x.recipe_id == product.id)[0].available * 100,
            ) / 100
        })
      })
  }
