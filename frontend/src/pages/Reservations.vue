  ...
  <div class="w-full">
    ...
    <Dialog v-model:visible="dialog" :header="$t('add_reservation')" :style="{ width: '450px' }">
      <div class="grid">
        ...
        <div class="col-6">
          <label>{{ $t('email') }}</label
          ><InputText v-model="form.customer_email" class="w-full" />
        </div>
        <div class="col-6">
          <label>{{ $t('date') }}</label
        ...
        <div class="col-6">
          <label>{{ $t('time') }}</label
          ><Calendar v-model="formTime" timeOnly class="w-full" />
        </div>
        <div class="col-6">
          <label>{{ $t('guests') }}</label
          ><InputNumber v-model="form.guest_count" :min="1" class="w-full" />
        </div>
        <div class="col-12">
          <label>{{ $t('notes') }}</label
    ...
    <Dialog v-model:visible="assignDialog" :header="$t('assign_table')" :style="{ width: '400px' }">
      <div class="grid">
        <div class="col-12">
          <label>{{ $t('table') }}</label
          ><InputText v-model="assignTable" class="w-full" />
        </div>
      </div>
      <template #footer>
...
const doSave = async () => {
  ...
  try {
    await axios.post(
      `http://${import.meta.env.VITE_APP_BACKEND_HOST}/reservations/api/reservations`,
      {
...
const doAssign = async () => {
  ...
  try {
    await axios.post(
      `http://${import.meta.env.VITE_APP_BACKEND_HOST}/reservations/api/reservations/${assignId.value}/assign`,
      { table: assignTable.value },
