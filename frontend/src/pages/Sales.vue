        ...
        <div class="grid w-full">
          ...
          <div class="col-12 flex justify-content-center align-items-center w-full">
            <div class="flex flex-column w-full">
                  ...
                  <Card style="max-width: 100%">
                    <template #content>
                      <Line
                        ...
                        style="min-height: 20rem; max-width: 100%"
                        :data="chartData"
                ...
                <div class="col-4 flex justify-content-start align-items-start">
                  <Card style="max-width: 100%">
                    <template #content>
                      <Pie
                        style="max-width: 100%"
                        v-if="productPiechartData != undefined"
                        :data="productPiechartData"
                        :chartOptions="productPiechartOptions"
                      />
                    </template>
                  </Card>
                </div>
              ...
              <DataTable
                @page="updatSalesTableRowsPerPage"
                :lazy="true"
                :totalRecords="salesTableTotalRecords"
                :loading="isSalesTableLoading"
                v-model:expandedRows="expandedSalesLogRows"
                paginatorPosition="both"
                paginator
                :rows="salesTableRowsPerPage"
              ...
              >
                ...
                <Column sortable field="costs" :header="$t('cost')">
                  <template #body="slotProps">
                    <div class="flex gap-2 align-items-center">
                      ...
                      <Badge
                        v-if="slotProps.data.inventory_refunds > 0"
                        :value="`-${slotProps.data.inventory_refunds}`"
                        severity="success"
                        class="mr-2"
                      />
                    </div>
                ...
                <Column sortable field="refunds_value" :header="$t('refunds')">
                  <template #body="slotProps">
                    <Badge
                      :value="`${slotProps.data.inventory_refunds > 0 ? '-' : ''}${slotProps.data.refunds_value}`"
                      severity="secondary"
                ...
                <Column sortable field="profit" :header="$t('profit')">
                  <template #body="slotProps">
                    <div class="flex align-items-center gap-2">
                      <Tag
                        :value="
                          slotProps.data.total_sales -
                          slotProps.data.costs -
                          slotProps.data.refunds_value +
                          (slotProps.data.inventory_refunds || 0)
                        "
                        :severity="
                          slotProps.data.total_sales -
                            slotProps.data.costs -
                      ...
                      />
                      <Badge
                        :value="`+${slotProps.data.tips} ${$t('tips')}`"
                        severity="secondary"
                        style="margin-right: 0.5rem"
                      />
                    </div>
                ...
                <template #expansion="slotProps">
                  <DataTable
                    v-model:expandedRows="expandedSalesLogOrderItems"
                  ...
                  >
                    ...
                    <Column sortable field="order.cost" :header="$t('cost')">
                      <template #body="slotProps">
                        <div class="flex gap-2 align-items-center">
                          ...
                          <Badge
                            v-if="orders_refunds[slotProps.data.id]?.inventory_refunds > 0"
                            :value="`-${orders_refunds[slotProps.data.id]?.inventory_refunds}`"
                            severity="success"
                            class="mr-2"
                          />
                        </div>
                    ...
                    <Column sortable field="order.sale_price" :header="$t('sales')">
                      <template #body="slotProps">
                        <div class="flex gap-2 align-items-center">
                          ...
                          <Badge
                            :value="slotProps.data.order.payment_source"
                            severity="secondary"
                            class="mr-2"
                          />
                        </div>
                    ...
                    <Column sortable field="profit" :header="$t('profit')">
                      <template #body="slotProps">
                        <div class="flex align-items-center gap-2">
                          <Tag
                            :value="
                              slotProps.data.order.sale_price -
                              slotProps.data.order.cost -
                              (orders_refunds[slotProps.data.id]?.total_refunds || 0) +
                              (orders_refunds[slotProps.data.id]?.inventory_refunds || 0)
                            "
                            :severity="
                              slotProps.data.order.sale_price -
                                slotProps.data.order.cost -
                          ...
                          />
                          <Badge
                            :value="`+${slotProps.data.order.tips} ${$t('tips')}`"
                            severity="secondary"
                            style="margin-right: 0.5rem"
                          />
                        </div>
                    ...
                    <template #expansion="slotProps">
                      <SalesLogTableItems
                        :items="slotProps.data.costs"
                        :order_refunds="orders_refunds[slotProps.data.id] || []"
                      />
                    </template>
                  </DataTable>
                </template>
...
const export_sales = (first = salesTableFirstIndex.value, rows = salesTableRowsPerPage.value) => {
  ...
  axios
    .get(
      `http://${import.meta.env.VITE_APP_BACKEND_HOST}${import.meta.env.VITE_APP_MODULE_CORE_API_PREFIX}/api/logs/salesperday/exportcsv?page[number]=${page_number}&page[size]=${rows}`,
      {
        responseType: 'blob',
        headers: {
      ...
      for (let i = 0; i < response.data.data.length; i++) {
        ...
        for (let j = 0; j < response.data.data[i].orders.length; j++) {
          ...
          for (let k = 0; k < response.data.data[i].orders[j].order.items.length; k++) {
            if (
              !productPieChartLabels.value.includes(
                response.data.data[i].orders[j].order.items[k].product.name,
              )
            ) {
              productPieChartLabels.value.push(
                response.data.data[i].orders[j].order.items[k].product.name,
              )
              productPieChartSales.value.push(
                response.data.data[i].orders[j].order.items[k].quantity,
              )
              productPieChartColors.value.push(
                '#' +
              ...
              const index = productPieChartLabels.value.indexOf(
                response.data.data[i].orders[j].order.items[k].product.name,
              )
              productPieChartSales.value[index] +=
                response.data.data[i].orders[j].order.items[k].quantity
            }
          }
        }
        ...
        for (let j = 0; j < response.data.data[i].refunds.length; j++) {
          ...
          if (orders_refunds.value[`${response.data.data[i].refunds[j].order_id}`]) {
            orders_refunds.value[`${response.data.data[i].refunds[j].order_id}`].refunds.push(
              refund,
            )
            orders_refunds.value[`${response.data.data[i].refunds[j].order_id}`].total_refunds +=
              refund.amount
            orders_refunds.value[`${response.data.data[i].refunds[j].order_id}`].inventory_refunds =
              0
          } else {
          ...
          if (refund.destination == 'inventory') {
            ...
            orders_refunds.value[
              `${response.data.data[i].refunds[j].order_id}`
            ].inventory_refunds += refund.item_cost
          }
          ...
          if (refund.destination == 'custom') {
            refund.material_refunds.forEach((material_refund) => {
              orders_refunds.value[
                ...
                material_refund.inventory_return_qty * material_refund.cost_per_unit
              day_inventory_refunds +=
                material_refund.inventory_return_qty * material_refund.cost_per_unit
            })
          }
        }
        ...
        chartProfit.value.push(
          temp_sales_log[i].total_sales -
            temp_sales_log[i].costs -
            temp_sales_log[i].refunds_value +
            (temp_sales_log[i].inventory_refunds || 0),
        )
      }
