    ...
    <div class="grid mx-2">
      <div class="col-12 flex">
        <div class="grid w-full">
          ...
          <div class="col-12">
            <DataTable
              @page="updatProductsTableRowsPerPage"
              :lazy="true"
              :totalRecords="productsTableTotalRecords"
              :loading="isProductsTableLoading"
              paginatorPosition="both"
              paginator
              :rows="productsTableRowsPerPage"
            ...
            <Dialog
              v-model:visible="productAddDialog"
              modal
            ...
            >
              <Form
                v-slot="$form"
                :initialValues="addproduct_initials"
                :resolver="add_product_resolver"
              ...
              >
                ...
                <div class="flex flex-column gap-2 mt-2">
                  ...
                  <FileUpload
                    ...
                    :url="`${backendUrl}/api/products/${new_product_id}/image`"
                ...
                <div class="flex flex-column gap-2 w-5 mt-2">
                  ...
                  <Message
                    ...
                    size="small"
                    variant="simple"
                    >{{ $form.ready.error?.message }}</Message
                  >
                </div>
                <div class="flex flex-column gap-2 w-10 mt-3">
                  ...
                  <DataTable :value="materials" stripedRows class="w-full pr-2">
                    ...
                    <Column field="quantity" :header="$t('quantity')">
                      <template #body="slotProps">
                        <InputNumber
                          v-model.number="materials[slotProps.data.index].quantity"
                          :min="0"
                          mode="decimal"
                          :maxFractionDigits="2"
                    ...
                    <template #header>
                      <div class="flex justify-start">
                        <Button
                          icon="pi pi-plus"
                          :label="$t('add_inventory_item', 1)"
                          rounded
                          raised
                          @click="add_material_dialog = true"
                        />
                      </div>
                    </template>
                  </DataTable>
                ...
                <div class="flex flex-column gap-2 w-10 mt-3">
                  ...
                  <DataTable :value="sub_products" stripedRows class="w-full pr-2">
                    ...
                    <Column field="quantity" :header="$t('quantity')">
                      <template #body="slotProps">
                        <InputNumber
                          v-model.number="sub_products[slotProps.data.index].quantity"
                          :min="0"
                          mode="decimal"
                          :maxFractionDigits="2"
                          aria-describedby="quantity"
                    ...
                    <template #header>
                      <div class="flex justify-start">
                        <Button
                          icon="pi pi-plus"
                          :label="$t('add_subproduct')"
                          rounded
                          raised
                          @click="add_subproduct_dialog = true"
                        />
                      </div>
                    </template>
                  </DataTable>
                ...
                <div v-if="newproduct_enable_fixed_cost" class="flex flex-column gap-2 w-5 mt-2">
                  <label for="fixed_cost">{{ $t('fixed_cost') }}</label>
                  <InputText
                    id="fixed_cost"
                    name="fixed_cost"
                    v-model.number="newproduct_fixed_cost"
                    aria-describedby="fixed_cost"
                  />
                </div>
            ...
            <Dialog
              v-model:visible="productEditDialog"
              modal
              :header="`${$t('edit_product')} ${productToEdit.name}`"
            ...
            >
              ...
              <div class="flex flex-column gap-2 mt-2">
                <FileUpload
                  @before-send="beforeEditProductImageUpload"
                  :chooseLabel="$t('edit')"
                  ref="editProductImageUpload"
                  name="image"
                  :fileLimit="1"
                  :showCancelButton="false"
                  :showUploadButton="false"
                  :url="`${backendUrl}/api/products/${productToEdit.id}/image`"
                  :multiple="false"
                  accept="image/*"
                  :maxFileSize="1000000"
                >
              ...
              <div class="flex flex-column gap-2 w-10 mt-3">
                ...
                <DataTable :value="productToEdit.materials" stripedRows class="w-full pr-2">
                  ...
                  <Column field="quantity" :header="$t('quantity')">
                    <template #body="slotProps">
                      <InputNumber
                        v-model.number="productToEdit.materials[slotProps.data.index].quantity"
                        :min="0"
                        mode="decimal"
                        :maxFractionDigits="2"
                        aria-describedby="quantity"
                  ...
                  <template #header>
                    <div class="flex justify-start">
                      <Button
                        ...
                        rounded
                        raised
                        @click="edit_material_dialog = true"
                      />
                    </div>
                  </template>
                </DataTable>
              ...
              <div class="flex flex-column gap-2 w-10 mt-3">
                ...
                <DataTable :value="productToEdit.sub_products" stripedRows class="w-full pr-2">
                  ...
                  <Column field="quantity" :header="$t('quantity')">
                    <template #body="slotProps">
                      <InputNumber
                        v-model.number="productToEdit.sub_products[slotProps.data.index].quantity"
                        :min="0"
                        mode="decimal"
                        :maxFractionDigits="2"
                        aria-describedby="quantity"
                  ...
                  <template #header>
                    <div class="flex justify-start">
                      <Button
                        ...
                        rounded
                        raised
                        @click="edit_subproduct_dialog = true"
                      />
                    </div>
                  </template>
                </DataTable>
              ...
              <div v-if="productToEdit.enable_fixed_cost" class="flex flex-column gap-2 w-5 mt-2">
                ...
                <InputText
                  ...
                  name="fixed_cost"
                  type="number"
                  v-model.number="productToEdit.fixed_cost"
                  aria-describedby="fixed_cost"
                />
              </div>
              <template #footer>
...
const add_product_resolver = ({ values }) => {
  ...
  if (!values.name) {
    errors.name = [{ message: t('validation_required') }]
  }

  if (!/^\d+(\.\d+)?$/.test(values.ready)) {
    errors.ready = [{ message: t('ready_invalid') }]
  }

  if (!/^\d+(\.\d+)?$/.test(values.price)) {
    errors.price = [{ message: t('price_invalid') }]
  }
...
const deleteProduct = (product_id: string) => {
  axios
    .delete(
      `http://${import.meta.env.VITE_APP_BACKEND_HOST}${import.meta.env.VITE_APP_MODULE_CORE_API_PREFIX}/api/products/${product_id}`,
      {
        headers: {
...
const submitProduct = () => {
  ...
  axios
    .post(
      `http://${import.meta.env.VITE_APP_BACKEND_HOST}${import.meta.env.VITE_APP_MODULE_CORE_API_PREFIX}/api/products`,
      {
        data: payload,
      },
      {
