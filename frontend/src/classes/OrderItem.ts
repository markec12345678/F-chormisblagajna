...
export class OrderItem {
    ...
    if (product != undefined) {
      ...
      this.materials = product.materials.map((material, index) => {
        ...
        material.entries.forEach((entry: MaterialEntry, entry_index) => {
          ...
          material_entries_total_cost_per_unit =
            material_entries_total_cost_per_unit + entry.purchase_price / entry.purchase_quantity
        })
        ...
        const itemmaterial = <OrderItemMaterial>{
          ...
          avgcost:
            (material_entries_total_cost_per_unit / material.entries.length) * material.quantity,
        }
  ...
  async FromItemData(orderItem: OrderItem) {
    ...
    this.materials.forEach((material: OrderItemMaterial, index: number) => {
      if (store.getSettings?.orders.default_cost_calculation_method == 'average')
        this.ValidateMaterialTotalQuantity(index)
      else this.ValidateMaterialExactQuantity(index)
    })
  ...
  async RefreshReadyNumber() {
    await axios
      .get(
        `http://${import.meta.env.VITE_APP_BACKEND_HOST}${import.meta.env.VITE_APP_MODULE_CORE_API_PREFIX}/api/products/${this.Id}`,
        {
          headers: {
  ...
  async RefreshProductData() {
    await axios
      .get(
        `http://${import.meta.env.VITE_APP_BACKEND_HOST}${import.meta.env.VITE_APP_MODULE_CORE_API_PREFIX}/api/products/${this.product.id}/recipetree`,
        {
          headers: {
            Authorization: `Bearer ${auth.accessToken.value}`,
          },
  ...
  async UpdateMaterialAverageCost(materialIndex: number) {
    await axios
      .get(
        `http://${import.meta.env.VITE_APP_BACKEND_HOST}${import.meta.env.VITE_APP_MODULE_CORE_API_PREFIX}/api/materials/${this.materials[materialIndex].material.id}/avgcost?quantity=${this.materials[materialIndex].quantity}`,
        {
          headers: {
            Authorization: `Bearer ${auth.accessToken.value}`,
          },
  ...
  async UpdateMaterialEntryExactCost(materialIndex: number) {
    if (this.materials[materialIndex].entry != undefined) {
      await axios
        .get(
          `http://${import.meta.env.VITE_APP_BACKEND_HOST}${import.meta.env.VITE_APP_MODULE_CORE_API_PREFIX}/api/materials/${this.materials[materialIndex].material.id}/entries/${this.materials[materialIndex].entry.id}/cost?quantity=${this.materials[materialIndex].quantity}`,
          {
            headers: {
              Authorization: `Bearer ${auth.accessToken.value}`,
            },
  ...
  ValidateItem() {
    ...
    this.materials.forEach((material) => {
      if (!material.isQuantityValid) valid = false
    })

    if (!this.ValidateSubItems()) valid = false
  ...
  ValidateAllMaterials() {
    for (let i = 0; i < this.materials.length; i++) {
      ...
      else this.ValidateMaterialExactQuantity(i)
    }
