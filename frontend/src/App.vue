  ...
  <main :style="`height:100%;direction:${orientation};`">
    ...
    <div
      ...
      style="
        ...
        width: 100%;
        height: 100%;
        background: linear-gradient(135deg, #14977b, #9cbf34);
      "
    ></div>
    <Dialog
    ...
    >
      <div class="mode-modal-wrapper">
        <div class="mode-modal-header">
          ...
          <p class="mode-modal-subtitle">
            {{ $t('this_setting_controls_which_features_are_available') }},
            {{ $t('you_can_change_it_later_in_settings') }}
          </p>
        </div>
        ...
        <section class="flex p-3 gap-3 w-full bg-black-alpha-90">
          ...
          >
            <i
              ...
              :class="`pi pi-check text-${message.severity}-500`"
            ></i>
            <i
              v-if="message.severity == `info`"
              :class="`pi pi-info text-${message.severity}-500`"
            ></i>
            <i
              v-if="message.severity == `warning`"
              :class="`pi pi-exclamation-triangle text-${message.severity}-500`"
            ></i>
            <i
              v-if="message.severity == `error`"
              :class="`pi pi-times text-${message.severity}-500`"
            ></i>
          </div>
...
.mode-card {
  ...
  &--selected {
    ...
    box-shadow:
      0 0 0 3px rgba(99, 241, 130, 0.2),
      0 8px 32px rgba(99, 241, 194, 0.15);
    &::before {
      opacity: 1;
    }
