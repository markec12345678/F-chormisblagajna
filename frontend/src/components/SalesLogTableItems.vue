  ...
  <DataTable v-model:expandedRows="expandedSalesLogOrderItemComponents" :value="localItems">
    ...
    <Column sortable field="ItemName" :header="$t('name')">
      <template #body="slotProps">
        <Badge
          :value="$t('refunded')"
          v-if="items_refunds[slotProps.data.item_id]"
          severity="danger"
        />
    ...
    <Column sortable field="Cost" :header="$t('cost')">
      <template #body="slotProps">
        <div class="flex gap-2 align-items-center">
          ...
          <Badge
            v-if="items_refunds[slotProps.data.item_id]?.inventory_refunds > 0"
            :value="`-${items_refunds[slotProps.data.item_id]?.inventory_refunds}`"
            severity="success"
            class="mr-2"
          />
        </div>
    ...
    <Column sortable field="refunds" :header="$t('refunds')">
      <template #body="slotProps">
        <Badge
          :value="`${items_refunds[slotProps.data.item_id]?.refund_amount}`"
          severity="danger"
          v-if="items_refunds[slotProps.data.item_id]?.is_refunded"
        />
      </template>
    </Column>
    ...
    <template #expansion="slotProps">
        ...
        <Column sortable field="cost" :header="$t('cost')">
          <template #body="slotProps">
            <div class="flex gap-2 align-items-center">
              {{ slotProps.data.cost }}
              <Badge
                :value="`-${items_refunds[slotProps.data.item_id]?.material_refunds[slotProps.data.component_id]?.inventory_return_qty * items_refunds[slotProps.data.item_id]?.material_refunds[slotProps.data.component_id]?.cost_per_unit}`"
                severity="success"
                v-if="
                  items_refunds[slotProps.data.item_id]?.material_refunds[
                    slotProps.data.component_id
        ...
        <Column sortable field="quantity" :header="$t('quantity')">
          <template #body="slotProps">
            <div class="flex gap-2 align-items-center">
              ...
              <Badge
                :value="`-${items_refunds[slotProps.data.item_id]?.material_refunds[slotProps.data.component_id]?.inventory_return_qty}`"
                severity="success"
                v-if="
                  items_refunds[slotProps.data.item_id]?.material_refunds[
                    slotProps.data.component_id
      ...
      <SalesLogTableItems
        v-if="slotProps.data.DownstreamCost != null"
        :items="slotProps.data.DownstreamCost"
      />
    </template>
  </DataTable>
</template>
        ...
        if (order_refund.order_item_id == item.item_id) {
          ...
          if (order_refund.destination == 'custom') {
            order_refund.material_refunds.forEach((material_refund) => {
              items_refunds.value[item.item_id].inventory_refunds +=
                material_refund.inventory_return_qty * material_refund.cost_per_unit
              items_refunds.value[item.item_id].material_refunds[material_refund.material_id] =
                material_refund
            })
          }
        }
