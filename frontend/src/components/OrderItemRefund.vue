  ...
  <div class="flex flex-column">
    <div class="flex w-full flex-column">
      ...
      <div class="flex w-full justify-content-start align-items-center">
        <Slider
          v-model="refund_value"
          :max="props.item.price"
          class="w-9 m-1 mx-2"
          style="height: 0.6rem"
        />
    ...
    <div class="flex gap-4">
      {{ props.item.product.name }} {{ $t('destination') }}
      <div class="flex flex-wrap gap-4">
        <div class="flex items-center gap-2">
          <RadioButton
            v-model="item_destination"
            inputId="option2"
            name="item_destination"
            value="inventory"
          />
          <label for="option2">{{ $t('inventory') }}</label>
        </div>
        <div class="flex items-center gap-2">
          <RadioButton
            v-model="item_destination"
            ...
            value="dispose"
          />
          <label for="option1">{{ $t('dispose') }}</label>
        </div>
        <div class="flex items-center gap-2">
          <RadioButton
            v-model="item_destination"
            ...
            value="waste"
          />
          <label for="option3">{{ $t('waste') }}</label>
        </div>
        <div class="flex items-center gap-2">
          <RadioButton
            v-model="item_destination"
            ...
            value="custom"
          />
          <label for="option4">{{ $t('custom_materials_products') }}</label>
        </div>
      </div>
    </div>
    <Divider />
    <Textarea v-model="reason" class="w-4" :placeholder="$t('reason')" />
    <h3 v-if="item_destination == 'custom'">{{ $t('materials') }}</h3>
    <div v-if="item_destination == 'custom'" class="flex flex-column">
      <div
        v-for="(material, index) in material_refunds"
        :key="index"
      ...
      >
        ...
        <div class="flex align-items-center gap-2">
          ...
          <InputText
            v-model.number="material_refunds[index]['inventory_return_qty']"
            type="number"
            aria-describedby="inventory_return_qty"
          />
        </div>
        <div class="flex align-items-center gap-2">
          <label for="dispose">{{ $t('dispose') }}</label>
          <InputText
            v-model.number="material_refunds[index]['dispose_qty']"
            type="number"
            aria-describedby="dispose_qty"
          />
        </div>
        <div class="flex align-items-center gap-2">
          <label for="waste">{{ $t('waste') }}</label>
          <InputText
            v-model.number="material_refunds[index]['waste_qty']"
            type="number"
            aria-describedby="waste_qty"
          />
        </div>
      </div>
    </div>
    ...
    <div class="flex align-items-center gap-2" v-if="item_destination == 'custom'">
      ...
      <Button
        ...
        size="small"
        severity="secondary"
        @click="pick_product_dialog = true"
      />
    </div>
    <div class="flex flex-column gap-3">
      ...
      >
        ...
        <div class="flex align-items-center gap-2">
          <InputText
            v-model.number="added_products_quantities[index]"
            type="number"
...
const submitRefund = () => {
  ...
  axios
    .post(
      `http://${import.meta.env.VITE_APP_BACKEND_HOST}${import.meta.env.VITE_APP_MODULE_CORE_API_PREFIX}/api/orders/${props.order.id}/items/${props.item.id}/refund?reason=${reason.value}`,
      {
        data: payload,
      },
      {
