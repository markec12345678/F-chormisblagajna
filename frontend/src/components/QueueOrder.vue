<template>
  <div :style="`display:${display};${state == 'cancelled' ? 'position:relative;' : ''}`">
    <div
      ...
      style="position: absolute; height: 100%; width: 100%; z-index: 9"
    ...
    <Dialog
      v-model:visible="product_details_visible"
      modal
      :header="`${$t('product_details')}  (${product_for_details?.name})`"
      :style="{ width: '75rem' }"
      :breakpoints="{ '1199px': '50vw', '575px': '90vw' }"
    >
      <ul>
        <li
          class="flex justify-content-between w-6 m-2"
          v-for="(material, index) in product_for_details?.materials"
          :key="index"
        >
...
const updateElapsedTime = () => {
  ...
  const updateTimer = () => {
    const now = new Date()
    const totalSeconds = differenceInSeconds(now, submittedAt)
    const hours = Math.floor(totalSeconds / 3600)
    const minutes = Math.floor((totalSeconds % 3600) / 60)
    const seconds = totalSeconds % 60
    timePassed.value = `${hours.toString().padStart(2, '0')}:${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')}`
  }
  updateTimer()
...
const confirmFinish = (event) => {
  confirm.require({
    ...
    message: t('confirm_finish_order'),
    ...
    accept: async () => {
      ...
      toast.add({
        ...
        detail: t('order_finished_detail'),
        life: 3000,
        group: 'br',
      })
    },
...
const startOrder = () => {
  axios
    .post(
      `http://${import.meta.env.VITE_APP_BACKEND_HOST}${import.meta.env.VITE_APP_MODULE_CORE_API_PREFIX}/api/orders/${props.order.id}/start`,
      {
        data: items.value,
      },
      {
...
const prepareOrder = async () => {
  ...
  for (let i = 0; i < props.order.items.length; i++) {
    ...
    item.materials.forEach((material, materialIndex) => {
      ...
      else item.ValidateMaterialExactQuantity(materialIndex)
    })
...
const display: boolean = computed(() => {
  ...
  if (state.value == 'finished') valid = 'none'
