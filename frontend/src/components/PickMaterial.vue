        ...
        <template #body="slotProps">
          <ButtonGroup>
            <Button
              ...
              severity="secondary"
              aria-label="Add"
              @click="returnMaterial(slotProps.data)"
            />
          </ButtonGroup>
        </template>
      </Column>
...
const returnMaterial = async (material: Material) => {
  axios
    .get(
      `http://${import.meta.env.VITE_APP_BACKEND_HOST}${import.meta.env.VITE_APP_MODULE_CORE_API_PREFIX}/api/materials/${material.id}/entries`,
      {
        params: {
          'page[number]': 0,
          'page[size]': 999,
